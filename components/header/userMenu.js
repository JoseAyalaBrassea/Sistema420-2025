      ...
      <nav className="flex flex-col w-full lg:w-auto lg:hidden lg:group-hover:flex lg:z-20 lg:text-center lg:absolute lg:right-0 lg:top-0 lg:max-w-none lg:bg-red-900 lg:text-xl lg:p-1 lg:mt-16 lg:border">
        {user.username ? (
          <button
            ...
            className="px-4 hover:cursor-pointer lg:text-white py-2 lg:px-3 w-full lg:hover:bg-white lg:hover:text-black whitespace-nowrap text-left"
          >
            Log Out
          </button>
        ) : (
              ...
              query: {
                lastPath:
                  router.pathname === "/login"
                    ? router.query.lastPath
                    : router.pathname,
              },
            }}
            className="px-4 hover:cursor-pointer lg:text-white py-2 lg:px-3 w-full lg:hover:bg-white lg:hover:text-black whitespace-nowrap"
          >
            Log In
          </Link>
        )}
        ...
        {(user.user_type === "ADMIN" || user.user_type === "admin") && (
          <Link
            href="/createUser"
            className="hover:cursor-pointer lg:text-white py-2 px-3 w-full lg:hover:bg-white lg:hover:text-black whitespace-nowrap"
          >
            Create User
          </Link>
        )}
