...
export default function PrintOptions({ setPrePrinting, setPrinting, isUnique, options, setOptions, printing}) {
   ...
   * Event handlers are provided for various user interactions including adding/removing tests,
...
const generateAndSendExcel = async () => {
  ...
  const shareByEmail = async (excelBuffer) => {
    ...
    const response = await fetch('/api/sendEmail', {
        ...
        body: JSON.stringify({
            email: email,  // Usa el email del estado que se actualiza en el input field
    ...
    if (response.ok) {
        ...
        setPrePrinting(false); // Cierra el modal o el menú de opciones después de enviar
    } else {
